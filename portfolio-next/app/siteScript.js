...
import { initShotCarousels } from './components/project/shotCarousel';
...
(function(){
  ...
  async function mountScene(canvas, color, count, radius, neural, nodeOpacity, lineOpacity, ptSize){
    ...
    let nodeMat = null, lineMat = null, nodeBaseOpacity = 0, lineBaseOpacity = 0;
    ...
    const sceneState = {running:false, scroll:0, scrollEased:0};
    ...
    function tick(){
      ...
      const revealT = Math.min(1, (performance.now() - revealStart) / 800);
      const reveal = 1 - Math.pow(1 - revealT, 3);

      // both axes ease toward their target (mouse position / idle spin) instead of snapping
      autoRotY += 0.0012;
      group.rotation.y += ((autoRotY + mx * 0.9) - group.rotation.y) * 0.05;
      group.rotation.x += (my * 0.28 - group.rotation.x) * 0.05;

      if (nodeMat) nodeMat.opacity = nodeBaseOpacity * reveal;
      if (lineMat) lineMat.opacity = lineBaseOpacity * reveal;
      ...
      if (canvas === heroCanvas){
        // ease the scroll progress itself (inertial, like the mouse parallax) instead
        // of snapping the transform straight to the raw scroll value every event
        sceneState.scrollEased += (sceneState.scroll - sceneState.scrollEased) * 0.08;
        group.rotation.z = sceneState.scrollEased * 0.55;
        scale *= (1 - sceneState.scrollEased * 0.4);
        camera.position.z = 6.4 + sceneState.scrollEased * 2.4;
      }
